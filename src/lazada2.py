...
def crawl_lazada(max_pages=5, delay=2):
    results = []

    for page in range(1, max_pages + 1):
        url = f"https://www.lazada.vn/tag/%C4%90i%E1%BB%87n-tho%E1%BA%A1i-di-%C4%91%E1%BB%99ng/?ajax=true&catalog_redirect_tag=true&from=hp_categories&isFirstRequest=true&page={page}&q=%C4%90i%E1%BB%87n%20tho%E1%BA%A1i%20di%20%C4%91%E1%BB%99ng&service=all_channel"
        ...
        # time out for prevent bot
        time.sleep(delay)

    return results

if __name__ == "__main__":
    data = crawl_lazada(max_pages=5, delay=2)  # rest 2 second each requests
    df = pd.DataFrame(data)
    df.to_csv("lazada_products_api.csv", index=False, encoding="utf-8-sig")
    print("Đã lưu kết quả vào lazada_products_api.csv")
